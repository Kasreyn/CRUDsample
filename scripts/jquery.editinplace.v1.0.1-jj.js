    ...
    return this.each(function() {
		...
		.click(function() {
		    ...
		    if (!editing) {
		        ...
		        var original_html = jQuery(this).html().trim();
		    ...
		    if (click_count == 1) {
		        ...
		        function saveAction() {
		            ...
		            if (settings.callback) {
						...
						var data = { "NewValue" : new_html, "ID" : original_element.attr("id") }
						//var data = "{'NewValue':'" + new_html + "','ID':'" + original_element.attr("id") + "'}"
						var datastring = JSON.stringify(data);
						/*$.post(settings.url, { data: datastring }, function showResult(res) {
                            alert(res);
                        });*/
		                ...
		                jQuery.ajax({
		                    ...
		                    //data: "{'NewValue':'" + new_html + "','ID':'" + original_element.attr("id") + "'}",
		                    //data: "NewValue=" + new_html + "&ID=" + original_element.attr("id"),
		                    ...
		                    success: function(data, status) {
								//alert(data.d+":"+status);		
		                        var new_text = data.d || settings.default_text;
		                        original_element.html(new_text);
		                        if (settings.success) {
		                            settings.success(data.d, original_element);
		                        }
		                    },
