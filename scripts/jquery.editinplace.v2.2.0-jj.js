...
$.extend(InlineEditor.prototype, {
	...
	createEditorElement: function() {
		...
		if ("select" === this.settings.field_type) {
			...
			//JJ: I want to display as text but have DB index in value. Also the numeric index is what should be sent to DB.
			//    default selected option does not work with edit.val(text) as it is the "text" value and not DB index
		}
	...
	createSelectEditor: function() {
		...
		for (var i=0; i<optionsArray.length; i++) {
			...
			var selected = (text == this.originalValue) ? 'selected="selected" ' : '';
	...
	handleCancelEditor: function(anEvent) {
		...
		/*if (hasContent(enteredText)	&& ! this.isDisabledDefaultSelectChoice() && ! isSelect)
	...
	handleSaveEditor: function(anEvent) {
		...
		var isSelect = false;
		var displaytext = "";
		var test = this.dom.find(':input.inplace_field');
		if ( test[0].type == "select-one") { isSelect = true; displaytext = test[0].options[test[0].selectedIndex].text; }
		...
		if (this.settings.callback)
			...
			this.handleSubmitToServer(enteredText,isSelect,displaytext);	
	},
	...
	handleSubmitToServer: function(enteredText,isSelect,displaytext) {
...
*/
		//$(this.dom.context.parentNode).children("[id*=ComputerID]")[0].innerHTML

		var data = { "NewValue" : enteredText }
		$(this.dom.context.parentNode).children().each(function(index) {
			data[$(this).attr("id")] = this.innerHTML;
		});
		data["ID"] = this.dom.attr("id");
...
//				var new_text = html || that.settings.default_text;
				var new_text = data.d || that.settings.default_text;

				/* put the newly updated info into the original element */
				// FIXME: should be affected by the preferences switch
				if (isSelect) { that.dom.html(displaytext); }
				else { that.dom.html(new_text); }
				// REFACT: remove dom parameter, already in this, not documented, should be easy to remove
				// REFACT: callback should be able to override what gets put into the DOM
				that.triggerCallback(that.settings.success, data.d);
			},
	...
	didStartSaving: function() {
		...
		//this.startSavingAnimation();  //JJ: crash here with stable jQuery 110118
	},
